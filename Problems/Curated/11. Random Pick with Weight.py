...
class Solution:
    ...
    def pickIndex(self) -> int:
        import random
        target_sum = random.randint(1, self.prefix)
        
        index = self.binarySearch(self.nums, target_sum)
        
        return index
    '''
    Time : O(N*logN) Construction of prefix array is O(N) and for pickIndex method is O(logN)
    Space : O(N) for keeping prefix sums and O(1) for binarySearch
